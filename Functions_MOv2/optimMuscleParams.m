function [osimModel_opt, SimInfo] = optimMuscleParams(osimModel_ref_filepath, osimModel_targ_filepath, N_eval, log_folder, limbSide, newDOFs, adjustOptFL)
...
if newDOFs(end-1) == '4'
    nM = 1;
elseif newDOFs(end-1) == '1'
    nM = 0;
end

for n_mus = 0:nM %muscles.getSize-1    
    tic
...
%     b = MTL_targ;
    A = LtenNorm_ref;
    b = MTL_targ - (LfibNormOnTen_ref*(LmOptLts(1)*Lopt));

    % ===== LINSOL =======
    % solving the problem to calculate the muscle param
    x = A\b;    
%     LmOptLts_opt(n_mus+1,:) = x;
    LmOptLts_opt(n_mus+1,:) = [LmOptLts(1)*Lopt , x];
