...
for n_mus = nMuscleForUse-1 %muscles.getSize-1 % remove first comment to do all muscle from one side. For course, only use 1 to save time.
    ...
    if curr_mus_name(end)=='l' && limbSide==1
        break;
    elseif curr_mus_name(end)=='r' && limbSide==0
        break;
    end
    
    disp(['processing mus ',num2str(n_mus+1),': ',char(curr_mus_name)])
    ...
    if min(x)<0
        % informing the user
        disp(['Negative value estimate for muscle parameter of muscle ',curr_mus_name])
        disp( '                         Lm Opt        Lts'   )
        disp(['Template model       : ',num2str(LmOptLts)])
        disp(['Optimized param      : ',num2str(LmOptLts_opt(n_mus+1,:))])
        ...
        if x(2)==0 
            if (max(Mus_ref(okList,3))-min(Mus_ref(okList,3)))<0.0001
                disp('Tendon length not changing throughout range of motion')
            end
...
diary off
    ...
    % PRINT LOGS
    disp('  ')
    disp(['Calculated optimized muscle parameters for ', char(curr_mus),' in ',num2str(toc),' seconds.'])
    disp( '                         Lm Opt        Lts'   )
    disp(['Template model       : ',num2str(LmOptLts)])
    disp(['Optimized param      : ',num2str(LmOptLts_opt(n_mus+1,:))])
    disp(['Nr of eval points    : ',num2str(evalOkPoints), '/',num2str(evalTotPoints),' used'])
    disp(['fval                 : ',num2str(fval)])
    disp(['var from template [%]: ',num2str(100*(abs(LmOptLts-LmOptLts_opt(n_mus+1,:)))./LmOptLts),'%'])
    disp(['root mean squared error: ',num2str(rmse), 'm'])
    disp('  ');
    diary off
