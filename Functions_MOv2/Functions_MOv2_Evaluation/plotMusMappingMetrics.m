...
%__________________________________________________________________________
%
function plotMusMappingMetrics(MusMapMetrics)

% angle of rotation of the text in the plots
rot_angle = -90;

% collecting information on muscles
n_mus = size(MusMapMetrics.colheaders,2);
MusMapMetrics.colheaders = strrep(MusMapMetrics.colheaders,'_',' ');

% first subplot: bars of RMSE
h1 = subplot(2,2,1:2);
bar(MusMapMetrics.RMSE,'DisplayName','RMSE',...
    'FaceColor',[0 0.498039215803146 0],'EdgeColor',[0 0 0]);
xlabel('Muscles');
ylabel('RMSE [L_m_,_n_o_r_m]')

set(gca,'xTick', 1:n_mus,'xTickLabel', MusMapMetrics.colheaders,'TickLength',[0 0]);%,'FontName','arial','FontSize',12
xlim([0 n_mus+1])
rotateXLabels( h1, rot_angle)

% second subplot: bars of RMSE
h2 = subplot(2,2,3:4);
bar(MusMapMetrics.MeanPercError,'DisplayName','MaxPercError',...
    'FaceColor',[0 0.498039215803146 0],'EdgeColor',[0 0 0]);
xlabel('Muscles');
